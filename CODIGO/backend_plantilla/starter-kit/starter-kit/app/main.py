...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Skip DB on startup: {settings.SKIP_DB_ON_STARTUP}")
    
    # Try to initialize database tables - but don't crash if it fails
    if not settings.SKIP_DB_ON_STARTUP:
        ...
        logger.info("Skipping database initialization (SKIP_DB_ON_STARTUP=true)")
